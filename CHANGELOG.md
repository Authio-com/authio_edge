...
All notable changes to `@useauthio/edge` are documented here. This
...
### Changed
- **Renamed npm package `@authio/edge` → `@useauthio/edge`.** The
  ...
  with `npm install @useauthio/edge` and update imports accordingly.
  The old `@authio/edge` name is retired; releases below this entry were
